...
@implementation V5ClientHanlder

+ (V5ClientHanlder *) shareInstance
{
    static V5ClientHanlder * manager = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        manager = [[V5ClientHanlder alloc] init];
    });
...
- (V5ChatViewController *)getChatViewController
{
    ...
    // 设置用户信息，坐席端可查看
    config.nickname = [UserCenter center].currentUser.store_name;
    config.gender = 0;
     ...
     */
    config.openId = [UserCenter center].currentUser.code;
...
//    config.userInfo = @{@"商品名称": @"牛仔裤", @"商品价格": @"¥168.00"};
    ...
    // 设置会话界面标题
    chatViewController.title = NSLocalizedString(@"客服-人工客服",nil); // 设置标题，默认@“小五”
    ...
    [chatViewController setClientOpenMode:ClientOpenModeAutoHuman withParam:nil];
    
    // 修改对话界面UI属性
    // 对话列表背景色
    chatViewController.chatTableView.backgroundColor = APGlobalUI.backgroundColor;
