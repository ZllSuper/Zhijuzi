...
- (void)viewDidLoad {
    ...
    // Toolbar
    _toolbar = [[UIToolbar alloc] initWithFrame:[self frameForToolbarAtOrientation:self.preferredInterfaceOrientationForPresentation]];
    ...
    if (_enableTapToDismiss) {
        tapGesture = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(dismissPhotoBrowser)];
...
- (void)performLayout {
    ...
    if ([self.navigationController.viewControllers objectAtIndex:0] == self) {
        ...
        if (self.rightItemTitle) {
            rightButtonTitle = self.rightItemTitle;
        }
        else {
            rightButtonTitle = NSLocalizedString(@"Done", nil);
        }
        _doneButton = [[UIBarButtonItem alloc] initWithTitle:rightButtonTitle style:UIBarButtonItemStylePlain target:self action:@selector(doneButtonPressed:)];
        // Set appearance
        ...
        if (self.rightItemTitle) {
            UIBarButtonItem *rightItem = [[UIBarButtonItem alloc] initWithTitle:self.rightItemTitle style:UIBarButtonItemStylePlain target:self action:@selector(rightBarButtonItemAction)];
...
{
    if ([self.delegate respondsToSelector:@selector(photoBrowser:rightItemButtonPressedForPhotoAtIndex:)]) {
        [self.delegate photoBrowser:self rightItemButtonPressedForPhotoAtIndex:self.currentIndex];
    }
}
